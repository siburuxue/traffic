...
$(function() {
    $('.back_report').on('click', function() {
        var the = $(this);
        var id = the.data('id');
        $('#hidden_report_id').val(id);
        $.post(url.reportBackStatus, {'id':id}, function(data) {
            if (data.status == 1) {
                approvalWin1 = layer.open({
                    type: 1,
                    closeBtn: 0,
                    area: ['600px', '240px'],
                    content: $('#approval-box'),
    ...
    $('#approval-reset').on('click', function() {
        $('textarea[name="back_reason"]').val('');
    });
    $('#js-close-jimmy').on('click', function() {
        //layer.close(approvalWin1);
        //var dialog1 = $('#approval-box');
        //dialog1.find('input[type=radio]').prop('checked', false);
        window.location.reload();
    });

    $('#approval-submit-jimmy').on('click', function() {
        var back_reason = $('textarea[name="back_reason"]').val();
        if (back_reason) {
            layer.confirm('是否确认执行该操作', {
                btn: ['是', '否'],
                closeBtn: 0
                ...
                //console.log(data);
                $.post(url.reportBack, data, function(data) {
                    if (data.status == 1) {
