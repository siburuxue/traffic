...
<head>
	...
	<script type="text/javascript">
		// 定义URL地址
		url.table = "<?php echo U('parkingTable');?>";
...
<body>
	...
	<div class="container-fluid main">
		...
		<div class="panel panel-default">
			...
			<div class="panel-heading">
				<div class="row">
					<div class="col-xs-6"><span class="block-title">车辆停放费用统计</span></div>
	...
	<table class="table table-bordered table-condensed table-form">
		<tr>
			<th width="15%">所属大队</th>
			<td colspan="3">
				<input type="text" class="form-control input-sm <?php if( false===is_power($myPower,'statistics_blood_3')): ?>disabled<?php endif; ?>" name="condition_department" readonly="readonly"
				data-target="#select-department" data-default="
				<?php if(false===is_power($myPower,'statistics_blood_3')): echo ($myBrigade["id"]); endif; ?>
				">
			</td>
		</tr>
		<tr>
			<th width="15%">办案人</th>
			<td width="35%">
				<input type="text" class="form-control input-sm <?php if( false===is_power($myPower,'statistics_blood_2,statistics_blood_3','or')): ?>disabled<?php endif; ?>" name="condition_handle"
				readonly="readonly" data-target="#select-handle" data-default="
				<?php if(false===is_power($myPower,'statistics_blood_2,statistics_blood_3','or')): echo ($my["id"]); endif; ?>
				">
			</td>
			<th width="15%">车号</th>
			<td>
		...
		<tr>
			<th>车型</th>
			<td>
				<select name="condition_grade_type" class="form-control input-sm search-auto">
					<option value="">请选择</option>
					<?php if(is_array($grade_type)): $i = 0; $__LIST__ = $grade_type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"><?php echo (str_replace('号牌','',$vo)); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</td>
			<th>停放地点</th>
			<td>
				<select name="condition_parking_lot" class="form-control input-sm search-auto">
					<option value="">请选择</option>
					<?php if(is_array($parking_lot)): $i = 0; $__LIST__ = $parking_lot;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</td>
		...
		<tr>
			...
			<td>
				<table width="100%">
					<tr>
						<td>
							<input type="text" class="form-control input-sm search-auto form-datetime" name="condition_detain_start_time" maxlength="16" readonly="readonly">
						</td>
						<td width="30" class="text-center">~</td>
						<td>
							<input type="text" class="form-control input-sm search-auto form-datetime" name="condition_detain_end_time" maxlength="16" readonly="readonly">
						</td>
					</tr>
				</table>
			</td>
			...
			<td>
				<table width="100%">
					<tr>
						<td>
							<input type="text" class="form-control input-sm search-auto form-datetime" name="condition_return_start_time" maxlength="16" readonly="readonly">
						</td>
						<td width="30" class="text-center">~</td>
						<td>
							<input type="text" class="form-control input-sm search-auto form-datetime" name="condition_return_end_time" maxlength="16" readonly="readonly">
						</td>
					</tr>
				</table>
			</td>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<table class="table table-striped table-bordered table-hover table-condensed table-sheet">
					<tr>
						<th width="50">编号</th>
						<th width="50%">车辆停放单位</th>
						<th>台次</th>
					</tr>
					<?php if(is_array($park)): $i = 0; $__LIST__ = $park;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
...
<div class="panel panel-default" style="position:absolute;z-index:9999;display:none;" id="select-handle">
	...
	<script type="text/javascript" src="<?php echo (SRC_MODULE_URL); ?>/js/Statistics/parking.js"></script>
