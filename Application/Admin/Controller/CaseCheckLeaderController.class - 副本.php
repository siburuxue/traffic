...
class CaseCheckLeaderController extends CommonController {
	...
	public function pendingTable() {
		...
		$list = $Model->where($map)->order('CaseCheck.create_time desc,CaseCheck.id desc')->limit($page->firstrow . ',' . $page->rows)->select();
		// 查询扩展信息
	...
	public function checkApprove() {
		...
		$data['check_user_id'] = I('post.check_user_id', '', 'int');
		$data['pid'] = $id;
