 ...
 * 办案人采血管(组)查询
 */
class CaseBloodtubeCateSearchController extends CommonController {
	...
	public function __construct() {
		parent::__construct();
		if (false === is_power($this->myPower, 'case_bloodtubecate_normal,case_bloodtubecate_advance,case_bloodtubecate_low', 'or')) {
			$this->error('没有权限');
		}
	...
	public function index() {
		if (true === is_power($this->myPower, 'case_bloodtubecate_advance')) {
			// 部门
			...
			$map = array();
			$map['is_del'] = 0;
			if (true === is_power($this->myPower, 'case_bloodtubecate_normal')) {
	...
	public function indexTable() {
		...
		//低级权限只能看到派发给自己的采血管数据
		if (true === is_power($this->myPower, 'case_bloodtubecate_low') && false === is_power($this->myPower, 'case_bloodtubecate_advance') && false === is_power($this->myPower, 'case_bloodtubecate_normal')) {
		...
		$list = $Model->where($map)->order($orderSort)->limit($page->firstrow . ',' . $page->rows)->group('BloodtubeCate.id')->select();
		...
		// 渲染
		$this->display('CaseBloodtubeCateSearch/index/table');
	}
