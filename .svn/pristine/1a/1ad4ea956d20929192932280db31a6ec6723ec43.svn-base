...
class AlarmReceiveController extends CommonController {
	...
	public function searchTable() {
		...
		switch ($orderField) {
		case 'accident_time':
			$orderby = 'Alarm.accident_time';
			break;
		case 'alarm_time':
			$orderby = 'Alarm.alarm_time';
			break;
		default:
			$orderby = 'Alarm.create_time';
			break;
		}
		if ($orderSort == 1) {
			$orderby .= ' asc';
		...
		$list = $Model->where($map)->order($orderby)->limit($page->firstrow . ',' . $page->rows)->group('Alarm.id')->select();
