$(function() {
    ...
    var renderAccept = function(accept) {
        if ($.isEmptyObject(accept)) {
            $('#caseAccept').attr('href', '#').addClass('disabled');
            $('#caseAcceptBtn').text('暂无受案登记');
            $('#caseAcceptContent').hide();
        } else {
            var link = url.caseAcceptHandleInfoDetail.replace('_accept_id_', accept.id);
            $('#caseAccept').attr('href', link).removeClass('disabled');
            $('#caseAcceptBtn').text('查看受案登记');
    ...
    var renderSurvey = function(survey, status) {
        if ($.isEmptyObject(survey)) {
            $('#caseSurvey').find('.list-group-item').each(function() {
                var the = $(this);
                the.attr('href', the.data('href').replace('_survey_id_', '')).addClass('disabled');
                the.toggleClass('list-group-item-info', false);
            });
        } else {
            $('#caseSurvey').find('.list-group-item').each(function() {
                var the = $(this);
                the.attr('href', the.data('href').replace('_survey_id_', survey.id)).removeClass('disabled');
    ...
    var renderClient = function(client) {
        ...
        if ($.isEmptyObject(client)) {
            ...
            $.each(client, function(i, data) {
                var link = url.caseClientInfoDetail.replace('_client_id_', data.id);
                html += '<a href="' + link + '" class="list-group-item js-open">';
    ...
    var renderRecord = function(record) {
        ...
        if ($.isEmptyObject(record)) {
            ...
            $.each(record, function(i, data) {
                var link = url.caseRecordHandleInfoDetail.replace('_record_id_', data.id);
                html += '<a href="' + link + '" class="list-group-item js-open">';
    ...
    var renderCheckup = function(checkup) {
        ...
        if ($.isEmptyObject(checkup)) {
            ...
            $.each(checkup, function(i, data) {
                var link = url.caseCheckupDetail.replace('_checkup_id_', data.id);
                html += '<a href="' + link + '" class="list-group-item js-open';
                if (data.is_red) {
