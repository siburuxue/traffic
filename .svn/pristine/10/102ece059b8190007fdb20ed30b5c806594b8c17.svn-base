 ...
 * 处罚
 */
class CasePunishHandleController extends CaseCommonController {
    /**
     * 画面加载
     */
     ...
     * 保存处罚信息
     */
    public function savePunishInfo(){
        $id = I('post.id','','int');
        $model = D('CasePunish');
        $model->startTrans();
        $data = $model->create();
        if($data['punish_fine_money'] == ''){
            $data['punish_fine_money'] = 0;
        }
        if($id == ""){
            $rs = $model->add($data);
        }else{
        ...
        if($rs === false){
            ...
            $map = array();
            $map['case_id'] = $this->case['id'];
            $punishList = M('CasePunish')->where($map)->select();
            $map = array();
            $map['id'] = $this->case['id'];
            if(count($clientList) == count($punishList)){
                $map['punish_complete_status'] = 1;
            }else{
                $map['punish_complete_status'] = 0;
            }
            ...
            $content = date('Y-m-d H:i')."对".$client['name']."予以".I('post.opinion');
