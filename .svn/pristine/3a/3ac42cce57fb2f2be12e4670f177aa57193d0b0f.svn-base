 ...
 * 受案信息 - 关联报警信息
 */
class FastCaseAlarmHandleController extends FastCommonController {
	...
	public function searchTable() {
		...
		// 渲染
		$this->display('FastCaseAlarmHandle/index/searchTable');
	}
	...
	public function linkedTable() {
		...
		// 渲染
		$this->display('FastCaseAlarmHandle/index/linkedTable');
	}
	...
	public function unlink() {
		...
		$map = array();
		$map['case_id'] = $caseId;
		$count = M('CaseAlarm')->where($map)->count();
		if($count == 0){
			$map = array();
			$map['id'] = $caseId;
			$map['accept_status'] = 0;
